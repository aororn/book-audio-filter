...
**Версия:** 10.7.0
**Дата:** 2026-01-29
**Статус:** Рабочий, улучшенная фильтрация v8.6
...
## Текущие показатели (v10.7.0)
- **Золотой стандарт глава 1: 31/31 (100%)** — 79 ошибок
- **Золотой стандарт глава 2: 21/21 (100%)** — 49 ошибок
- **Золотой стандарт глава 3: 20/20 (100%)** — 83 ошибки
- **Золотой стандарт глава 4: 21/21 (100%)** — 51 ошибка
- **Общий золотой стандарт: 93/93 (100%)**
- **Ложные срабатывания (FP): 262** (было 324, улучшение на 19%)
- **Словарь имён: 550 базовых → 9488 форм** (автозагрузка)
- **Строк кода: ~15 000** (Python 3.12+, оптимизировано)
- **Unit-тесты: 370+** (pytest, 0 failures)

## v10.7 — Улучшенная фильтрация

Новое в v10.7:
- **safe_ending_transition** — новый фильтр на основе анализа БД (63 FP отфильтровано)
- **Исправлен mark_golden()** — корректная разметка Golden с нормализацией регистра
- **FP снижены на 19%** — с 324 до 262
- **Recall фильтрации: 72.3%** (было 64.1%)

| Модуль | Версия | Интеграция | Назначение |
|--------|--------|------------|------------|
| **smart_compare.py** | v10.5 | основной | Умное сравнение + метаданные версий |
| **engine.py** | v8.6 | filters/ | Фильтрация + safe_ending_transition |
| **false_positives_db.py** | v3.1 | Инструменты/ | БД ложных срабатываний |
| **run_full_test.py** | v6.2 | Тесты/ | Тестирование + флаг --clean |
| **AlignmentManager** | v1.2 | smart_compare.py | Макро-выравнивание через якоря + монотонность |
| **ComparisonStitcher** | v1.1 | smart_compare.py | Склейка разбитых слов (средо+точие) |
| **WindowVerifier** | v1.0 | smart_compare.py | Контекстная верификация сегментов |
| **ScoringEngine** | v1.2 | engine.py | Адаптивные штрафы, защита имён |
| **MorphoRules** | v1.1 | engine.py | Консервативная морфологическая фильтрация |
| **CharacterGuard** | v1.0 | scoring_engine.py | Защита имён персонажей |

Текущая рабочая версия: **v10.5.0**
...
│   ├── smart_compare.py v10.1  # Умное сравнение с посегментным выравниванием
│   ├── golden_filter.py v5.0.0 # Обёртка обратной совместимости → filters/
│   ├── filters/                # Модульный пакет фильтрации v8.1
...
│   │   ├── engine.py v8.1      # Движок фильтрации (20 уровней) + ScoringEngine
│   │   ├── morpho_rules.py v1.1 # Консервативные морфологические правила
│   │   ├── character_guard.py v1.0 # Защита имён персонажей (интегрирован)
│   │   ├── scoring_engine.py v1.2  # Адаптивные штрафы (интегрирован)
│   │   └── window_verifier.py v1.0 # Верификация сегментов (интегрирован)
│   ├── alignment_manager.py v1.2   # Макро-выравнивание + фильтр монотонности
...
│   ├── run_full_test.py v6.0          # Полный тест системы с архивированием
│   ├── golden_test_history.json       # История результатов тестов
│   ├── История/                       # Архив прогонов (отдельные файлы)
...
│   └── золотой_стандарт_глава4.json   # Эталонные ошибки главы 4 (22)
...
### 4.5. Архитектура v9.2 — Полная интеграция модулей
...
├─────────────────────────────────────────────────────────────────┤
│ 1. ComparisonStitcher v1.1│ Склейка разбитых слов               │
│    ("средо"+"точие")      │ + PROTECTED_SHORT_WORDS (27 слов)   │
├───────────────────────────┼─────────────────────────────────────┤
│ 2. AlignmentManager v1.1  │ Макро: якоря + сегменты             │
│    (18.4% слов)           │ + refine_large_segments(>100 слов)  │
├───────────────────────────┼─────────────────────────────────────┤
│ 3. SequenceMatcher        │ Микро: выравнивание внутри сегментов│
│    (per-segment)          │ + compare_with_anchors()            │
├───────────────────────────┼─────────────────────────────────────┤
│ 4. WindowVerifier v1.0    │ Контекстная верификация ≥95%        │
│    (интегрирован)         │ TECHNICAL_OK / SUSPICIOUS / ERROR   │
├───────────────────────────┼─────────────────────────────────────┤
│ 5. ScoringEngine v1.2     │ Адаптивные штрафы + HARD_NEGATIVES  │
│    (интегрирован)         │ +100 имена, чистые 3 пары имён      │
└───────────────────────────┴─────────────────────────────────────┘
                                     │
                                     ▼
┌─────────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────────┤
│ УРОВЕНЬ -1: ScoringEngine │ Защита HARD_NEGATIVES (имена)       │
├───────────────────────────┼─────────────────────────────────────┤
│ УРОВЕНЬ 0: MorphoRules    │ Консервативная морфология           │
│    v1.1 (интегрирован)    │ Разные лемма/число/падеж = реальное │
├───────────────────────────┼─────────────────────────────────────┤
│ УРОВНИ 1-20               │ Статические правила фильтрации      │
└───────────────────────────┴─────────────────────────────────────┘
```

**ComparisonStitcher v1.1** (в `smart_compare.py`):
- Склеивает разбитые Яндексом слова: "средо" + "точие" → "средоточие"
- `PROTECTED_SHORT_WORDS` — 27 защищённых слов, которые НЕ склеиваются
- Предотвращает ложную склейку: "кто" + "то" остаётся раздельно

**AlignmentManager v1.1** (`alignment_manager.py`):
- Находит уникальные слова в обоих текстах (якоря)
- `refine_large_segments()` — разбивает сегменты >100 слов суб-якорями
- `check_density()` — проверка плотности сегментов
- Результат на главе 1: 1473 якоря, 1205 сегментов

**WindowVerifier v1.0** (`filters/window_verifier.py`) — **ИНТЕГРИРОВАН**:
- Контекстная верификация: проверяет окно ±3 слова без пробелов
- Схожесть ≥95% = TECHNICAL_OK (можно скрыть как шум)
- Консервативный подход: при сомнениях — показывать человеку

**ScoringEngine v1.2** (`filters/scoring_engine.py`) — **ИНТЕГРИРОВАН**:
- Интегрирован в smart_compare.py И engine.py
- Штрафы: +100 имена, +80 hard negatives, +40 разные леммы
- `HARD_NEGATIVES` — только 3 пары имён (джейра→вчера, рагедон↔рагидон)
- Архитектура: грамматика обрабатывается MorphoRules, НЕ костылями

**MorphoRules v1.1** (`filters/morpho_rules.py`) — **ИНТЕГРИРОВАН**:
- Консервативная морфологическая фильтрация
- Разная лемма → НЕ фильтровать
- Разная часть речи (VERB↔GRND) → НЕ фильтровать
- Разное число/падеж/время → НЕ фильтровать
- Одинаковая форма → ФИЛЬТРОВАТЬ

**CharacterGuard v1.0** (`filters/character_guard.py`) — **ИНТЕГРИРОВАН**:
- Загружает 550 базовых имён → 9488 падежных форм
- Используется ScoringEngine для расчёта штрафов
- Исправлен баг: комментарии (#) теперь пропускаются при загрузке
...
## История версий
- **v9.2.0** (2026-01-29) — ТЕКУЩАЯ ВЕРСИЯ. Полная интеграция всех модулей: WindowVerifier в smart_compare, ScoringEngine в engine.py, очищены HARD_NEGATIVES (только 3 пары имён). Golden: 93/93 (100%). FP: 357.
- **v10.0.0** (2026-01-29) — Посегментное выравнивание: compare_with_anchors(), merge_adjacent_opcodes(), refine_large_segments().
- **v9.1.0** (2026-01-29) — Исправлена регрессия ComparisonStitcher: PROTECTED_SHORT_WORDS (27 слов).
- **v9.0.0** (2026-01-29) — Экспериментальная система якорей. Модули созданы, но требовали интеграции.
- **v8.1.1** (2026-01-29) — Исправлен баг с именами собственными в morpho_rules.py. Golden: 94/94 (100%). FP: 356.
- **v8.1.0** (2026-01-29) — Переход на pymorphy3 (Python 3.14 совместимость)
- **v8.0.0** (2026-01-26) — Консервативная фильтрация: morpho_rules.py. Золотой стандарт глава 4 (22 ошибки). Итого: 94/94.
- **v6.0** тест (2026-01-29) — run_full_test.py v6.0: архивирование прогонов в Тесты/История/, автоопределение версии фильтров, поддержка главы 4, команды --versions/--archive.
- **v5.7** (2026-01-25) — Лучший результат: 183 FP (62+42+79) при 72/72 golden.
