...
### v10.7 — Улучшение фильтрации: safe_ending_transition

**Цель:** Снизить FP без потери Golden через анализ паттернов окончаний.

#### Новый фильтр в engine.py v8.6.0

**safe_ending_transition** — фильтрует FP с безопасными переходами окончаний:
- Выявлено 79 уникальных переходов окончаний, которые встречаются ТОЛЬКО в FP, НИКОГДА в Golden
- Условия срабатывания: `same_lemma=1` + `same_POS` + безопасный переход окончания

Примеры безопасных переходов:
- `ие→ия`, `ия→ие` — падежи существительных на -ие/-ия
- `ые→ое`, `ая→ые` — число прилагательных
- `ит→ят`, `ят→ит` — лицо глаголов 3л.

#### Исправления

1. **false_positives_db.py** — исправлен `mark_golden()` для нормализации регистра
   - Golden в файлах в верхнем регистре (ТЕРЯЮ), в БД в нижнем (теряю)
   - Теперь находит 85/88 Golden паттернов (было 73)

#### Результаты

| Метрика | v8.5.1 | v8.6.0 | Изменение |
|---------|--------|--------|-----------|
| Golden | 93/93 ✓ | 93/93 ✓ | = |
| FP | 324 | 262 | **-62 (-19%)** |
| FP Recall | 64.1% | 72.3% | **+8.2%** |
| safe_ending_transition | — | 63 | новый |

**Вывод:** Значительное улучшение без потери качества.

---

## [10.6.0] - 2026-01-29

### v10.6 — Тестирование точности фильтров и удаление вредных фильтров

**Цель:** Создать систему тестирования фильтров на основе БД и убрать фильтры, удаляющие golden ошибки.

#### Новые файлы

1. **false_positives_db.py v3.0** — расширенная схема БД
   - Новые колонки: `actual_filter`, `expected_filter`, `filter_correct`, `transcript_count`, `is_stable`, `chapter`
   - Методы: `add_error_with_filter()`, `get_unfiltered_fps()`, `get_filter_accuracy()`, `clear_all()`, `update_stability()`, `set_expected_filter()`
   - CLI команды: `filter-stats`, `unfiltered`, `clear`, `set-expected`

2. **collect_all_errors.py v1.0** — сбор ошибок со всех транскрипций
   - Прогоняет smart_compare + engine на 16 транскрипциях
   - Собирает 5062 вхождения, 854 уникальных паттерна
   - Автоматически помечает golden и FP

3. **test_filter_accuracy.py v1.0** — pytest тесты точности фильтров
   - `test_fp_recall_minimum` — минимум 60% FP должны фильтроваться
   - `test_golden_not_filtered` — **0 golden ошибок должны фильтроваться**
   - `test_*_coverage` — покрытие по категориям

#### Отключённые фильтры (engine.py v8.5.1)

Анализ показал 4 фильтра, которые удаляют больше golden чем FP:

| Фильтр | Golden удалено | FP удалено | Статус |
|--------|----------------|------------|--------|
| `yandex_conjunction_before_gerund` | 51 | 0 | ❌ ОТКЛЮЧЁН |
| `yandex_particle_deletion` | 20 | 0 | ❌ ОТКЛЮЧЁН |
| `duplicate_word_insertion` | 51 | 3 | ❌ ОТКЛЮЧЁН |
| `compound_prefix` | 51 | 17 | ❌ ОТКЛЮЧЁН |
| `split_pair_yandex` | 11 | 11 | ❌ ОТКЛЮЧЁН |

**Причина:** Эти фильтры предполагали, что insertion/deletion коротких слов — всегда ошибка Яндекса. Но чтец тоже может вставить/пропустить слово.
...
| Golden | 93/93 ✓ | 93/93 ✓ |
| FP | 309 | 324 |
| Golden отфильтровано | 3 | 0 |
| Тесты точности | — | 11/11 ✓ |

**Важно:** FP увеличились на 15, но теперь ни одна реальная ошибка не удаляется.
...
**Цель:** Обеспечить консистентность тестирования и убрать дублирование кода.

#### Проблемы (исправлены)

1. **Тестирование на старых файлах** — при изменении smart_compare.py тесты могли использовать закэшированные compared.json от старых версий
2. **Путаница с версиями** — непонятно, какой версией модуля создан файл
3. **Дублирование логики** — функция `is_alignment_artifact` была и в smart_compare.py, и в engine.py

#### Решения

1. **Метаданные версий в JSON** (smart_compare.py v10.5)
   ```json
   {
     "metadata": {
       "created_at": "2026-01-29T22:10:44",
...
2. **Валидация версий** (engine.py v8.5)
   - `MIN_SMART_COMPARE_VERSION = '10.5.0'`
   - Предупреждение если compared.json создан старой версией
   - Рекомендация пересоздать с `--force`

3. **Флаг `--clean`** (run_full_test.py v6.2)
   - Удаляет все compared/filtered файлы перед пайплайном
   - Гарантирует тестирование на свежих файлах
   - `python Тесты/run_full_test.py --clean`

4. **Удалено дублирование** (smart_compare.py v10.5)
   - Функция `is_alignment_artifact` удалена из smart_compare.py
   - Вся логика артефактов теперь только в engine.py
   - Единая точка контроля фильтрации
...
#### Файлы изменены
- `Инструменты/smart_compare.py` v10.1 → v10.5 (метаданные, удалено дублирование)
- `Инструменты/filters/engine.py` v8.0 → v8.5 (валидация версий)
- `Тесты/run_full_test.py` v6.1 → v6.2 (флаг --clean)
