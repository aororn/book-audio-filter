...
YANDEX_NAME_ERRORS: Set[Tuple[str, str]] = {
    ...
    ('куровал', 'урвал'),
    # v11.2 — из анализа БД (Этап 1)
    ('големон', 'големом'), ('големой', 'големы'),  # голем — фэнтези-существо
    ('соглядатая', 'согладатая'),  # опечатка в оригинале или ошибка Яндекса
    ('властелю', 'властелину'),  # властелин
    ('рагидон', 'рагедон'),  # имя персонажа
}
...
# v11.8: Перемещено из engine.py — однобуквенные символы, которые НЕ являются артефактами
# Эти слова при разбиении слова (големы → голе + мы) не должны фильтроваться
SKIP_SPLIT_FRAGMENT: Set[str] = {'и', 'а', 'я', 'о', 'у', 'в', 'с', 'к'}
...
YANDEX_TYPICAL_ERRORS: Set[Tuple[str, str]] = {
    ...
    # v11.2 — из анализа БД (Этап 1)
    ('завуалированно', 'завуалировано'),  # вариант написания
    ('криву', 'криво'),  # Яндекс: у→о
    ('плетены', 'вплетены'),  # Яндекс потерял приставку "в"
    ('кратчайше', 'кратчайший'),  # Яндекс: окончание
    # v8.8 — из анализа БД (дополнительные безопасные пары diff_lemma)
    ('средина', 'середина'), ('середина', 'средина'),  # вариант написания
    ('уж', 'уже'), ('уже', 'уж'),  # вариант слова
    ('вбивают', 'выбивают'), ('выбивают', 'вбивают'),  # приставка в/вы
    ('ускользнуть', 'скользнуть'), ('скользнуть', 'ускользнуть'),  # приставка у
    ('проводить', 'проходить'), ('проходить', 'проводить'),  # похожие глаголы
    ('идти', 'прийти'), ('прийти', 'идти'),  # вариации глагола
}
