"""
SemanticManager v2.1 — Менеджер семантических векторов для детекции оговорок.
...
Порог семантической близости:
    - В engine.py используется SEMANTIC_SLIP_THRESHOLD = 0.4
    - similarity >= 0.4: вероятно оговорка чтеца (защищено от фильтрации)
    - similarity < 0.4: скорее ошибка STT (может быть отфильтровано)
...
v2.1.0 (2026-01-31): Пороги из config.py
v2.0.0 (2026-01-30): Navec вместо gensim
"""
...
# v2.1: Порог из config.py
from .config import get_synonym_threshold

# Алиас для обратной совместимости
SYNONYM_THRESHOLD = 0.5  # Используй get_synonym_threshold()
