...
КЛЮЧЕВОЕ ИЗМЕНЕНИЕ v3.0:
    ...
    Если лемма одинаковая, но есть грамматические различия — это МОЖЕТ быть реальная ошибка.
    Финальное решение (Golden vs FP) принимает engine.py через словари и HARD_NEGATIVES.

ПРИЗНАКИ РЕАЛЬНОСТИ (добавляют баллы):
    +100: Имя персонажа (character_shield)
    +70:  Разные леммы (different_lemma) — разные слова!
    +70:  Грамматическое различие при одной лемме (grammar_change)
...
ПРИЗНАКИ ЛОЖНОСТИ (вычитают баллы):
    ...
    -40:  Полностью одинаковая форма (same_form)
...
class SmartScorer:
    ...
    def apply_morphology(
        ...
        ФИЛОСОФИЯ: SmartFilter даёт базовую оценку, engine.py уточняет.

        ЛОГИКА:
        - Разные леммы = разные слова = +70 (реальная ошибка)
        - Одинаковая лемма + грамм. различия = +70 (может быть Golden!)
        - Полностью одинаковая форма = -40 (FP)
        - Разная POS = +40 (доп. признак реальности)

        ВАЖНО: Разделение Golden vs FP для same_lemma делается в engine.py,
        а не здесь, т.к. требует контекста (HARD_NEGATIVES, словари и т.д.)
        ...
        if not same_lemma:
            ...
            if has_grammar_diff or not same_pos:
                # Грамматические различия — МОЖЕТ быть реальная ошибка
                # Финальное решение примет engine.py
