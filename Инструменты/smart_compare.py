...
# Пары слов, которые Яндекс часто путает из-за фонетического сходства
YANDEX_PHONETIC_PAIRS: frozenset = frozenset({
    # Частицы и союзы
    ('не', 'ни'), ('ни', 'не'),
    ('ну', 'но'), ('но', 'ну'),
    ('а', 'о'), ('о', 'а'),
...
def is_yandex_phonetic_error(word1: str, word2: str) -> bool:
    """
    Проверяет, является ли пара слов типичной фонетической ошибкой Яндекса.
    """
    pair = (word1.lower(), word2.lower())
    return pair in YANDEX_PHONETIC_PAIRS


# =============================================================================
# ALIGNMENT ARTIFACT DETECTOR v10.4 — Детекция артефактов выравнивания
# =============================================================================

# Частицы в составных словах — НЕ артефакты выравнивания
COMPOUND_PARTICLES = frozenset({'то', 'нибудь', 'либо', 'кое', 'таки'})


def is_alignment_artifact(orig: str, trans: str) -> bool:
    """
    Детектирует артефакты выравнивания — ложные ошибки из-за неточного
    сопоставления позиций слов.

    v10.4: Добавлена проверка лемм — грамматические различия НЕ артефакты
    v10.3: Исключены частицы составных слов (то/нибудь/либо и т.д.)
    """
    o = orig.lower().strip()
    t = trans.lower().strip()
    ...
    # v10.3: Исключаем частицы составных слов — это реальные замены
    # "какому-то" → "какому-нибудь" — это НЕ артефакт
    if o in COMPOUND_PARTICLES or t in COMPOUND_PARTICLES:
        return False

    len_o, len_t = len(o), len(t)

    # Паттерн 1: Короткое слово (≤2) vs длинное (≥5)
    if (len_o <= 2 and len_t >= 5) or (len_t <= 2 and len_o >= 5):
        return True
...
# =============================================================================
# COMPARISON STITCHER v1.1 — Склейка разбитых слов (v9.0, обновлён v10.2)
# =============================================================================
...
class ComparisonStitcher:
    ...
    # v10.2: Расширенный список защищённых коротких слов
    ...
    PROTECTED_SHORT_WORDS = frozenset({
        ...
        # v10.2: Новые защищённые слова (из анализа FP)
        'но', 'ну', 'да', 'из', 'под', 'над', 'при', 'про',
        # Местоимения (короткие)
        'он', 'мы', 'вы', 'ты', 'им', 'их', 'ей', 'ем', 'её',
        # Междометия
        'ох', 'ах', 'эх', 'хм', 'эй', 'ой', 'ух', 'эм',
    })
        ...
        if tag == 'equal':
            ...
            for orig_local_idx, trans_local_idx, pair_sim in smart_pairs:
                ...
                if orig_word and trans_word:
                    ...
                    # v10.2: Проверяем фонетические пары Яндекса
                    if not is_yandex:
                        if is_yandex_phonetic_error(orig_word.text, trans_word.text):
                            is_yandex = True

                    # v10.2: Проверяем артефакты выравнивания
                    if not is_yandex:
                        if is_alignment_artifact(orig_word.text, trans_word.text):
                            is_yandex = True
